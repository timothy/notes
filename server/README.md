...
Reference implementation of the Notes API contract in [`../openapi.yaml`](../openapi.yaml), version 2.0.0. The contract is the source of truth: request bodies are validated by the spec's own JSON Schemas, typed models are generated from the document, and every response in the test suite is checked against it. Every operation of the contract is implemented; the plan, the task list, and the closing work (the negative-case replay and the documentation pass) are in [`../tasks/`](../tasks/).
...
- Request comments: accepted on closed requests while the note is active, for create, edit, and delete alike; on a trashed note owners still list and read them while every request-comment mutation is `409 note_not_active` after the `412` check and everyone else is `404`; the inspect rule governs (read access suffices; `comment` and `propose_edit` are irrelevant); an owner who authored a request comment edits it as its author; a comment reached through another request is `404` before any `403`.
...
The inspectors of an edit request, its note's owners and its proposer while they can still read the note, talk about it under `GET/POST /edit-requests/{requestId}/comments` and `GET/PATCH/DELETE /edit-requests/{requestId}/comments/{commentId}`; everyone else is `404`. No note `comment` permission is needed, so a propose-only proposer can answer review feedback. Request comments have their own ETags (`PATCH` and `DELETE` require `If-Match`), sort oldest first, must belong to the request in the path, are accepted on closed requests while the note is active, and never advance the request's ETag or the note's. The author edits their own comment and any owner deletes any; a trashed note freezes them (`409 note_not_active`) while its owners still read them.
